...
Схема API будет примерно такой (позже данная структура может модифицироваться):
```
Request:
/image_undestanding/predict
{
...
Запуск инференс сервера происходит с помощью bash скрипта [`run_model_serving.sh`](../scripts/run_model_serving.sh)
